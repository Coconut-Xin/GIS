  ...
  <div class="container">
    ...
    <div id="map">
      ...
      <div class="map-container2">
        <span class="updateInfo">通信更新：</span>
...
export default {
  ...
  data() {
    return {
      switchValue: ref(false),//开关状态
      randomData: [],
      featureList: [],//point的feature列表
      lineFeatureList: [],//line的feature列表
      lineFeatureListForDamage: [],//失联line的feature列表

      lineNewFeatureList: [],//lineNew的feature列表
      lineNewFeatureListForDamage: [],//失联lineNew的feature列表
      ...
      lineNewVector: null,//lineNew的vector
      lineNewVectorForDamage: null,//lineNewForDamage的vector
  ...
  methods: {
    ...
    initWebSocket() {
      ...
      this.ws.onmessage = (event) => {
        ...
        this.nodeNewDataList = this.message.nodeNewDataList;
        ...
        if (!this.isFirstMessageReceived && this.nodeNewDataList != null) {
          this.isFirstMessageReceived = true;
          this.drawNewLine();

        }

        console.log('Message from server_this.message.nodeList:', this.message.nodeList);
        console.log('Message from server_this.message.nodeNewDataList:', this.message.nodeNewDataList);
    ...
    */
    ...
    getLinePositions() {
      ...
      var clusterNewPositions = [];
      var clusterNewPositionsForDamage = [];

      //针对每个集群数据
      this.clusterData.forEach((item, index) => {
        var position = [];
        var nextposition = [];
        var positions = [];//正常通信边positions
        var positionsForDamage = [];//失联通信边positions
      ...
      //新通信数据的position
      if (this.nodeNewDataList != null) {
        var positionsNew = [];//新数据，正常通信边positions
        var positionsNewForDamage = [];//新数据，失联通信边positions
        this.nodeNewDataList.forEach((itemNode, index) => {
          //针对每个节点，遍历通信节点列表，在Point中找到各节点位置
          var pointfeatures = this.layervector.getSource().getFeatures();
          var srcpoint = pointfeatures.find(feature => feature.getProperties()["vehicleId"] === itemNode.agentId);
          var position = srcpoint.getGeometry().getCoordinates();
          var destpoint = pointfeatures.find(feature => feature.getProperties()["vehicleId"] === itemNode.upEntityID);
          var nextposition = destpoint.getGeometry().getCoordinates();
          //判断通信状态
          if (itemNode.commState === "通信断开") {
            positionsNewForDamage.push([position, nextposition]);
          } else {//正常通信
            positionsNew.push([position, nextposition]);
          }
        });
        clusterNewPositions.push(positionsNew);
        clusterNewPositionsForDamage.push(positionsNewForDamage);
      }
      linePositions.push(clusterPositions, clusterPositionsForDamage, clusterNewPositions, clusterNewPositionsForDamage);
    ...
    drawLine() {
      ...
      var clusterNewPositions = linePositions[2];
      var clusterNewPositionsForDamage = linePositions[3];
    ...
    drawNewLine() {
      ...
      var clusterNewPositionsForDamage = linePositions[3];
      //正常通信线条绘制
      if (clusterNewPositions.length > 0) {
        clusterNewPositions.forEach((itemPositons, index) => {
          // var _that = this;
          var line = new Feature({
            geometry: new MultiLineString(itemPositons),
          });
          ...
          this.lineNewFeatureList.push(line);
          this.lineNewVector = new VectorLayer({
            source: vectorSource,
            name: "lineNew",
            style: new Style({
              stroke: new Stroke({
          ...
          this.lineNewVector.setVisible(this.switchValue);
          this.map.addLayer(this.lineNewVector);
        });
      }
      //失联通信线条绘制
      if (clusterNewPositionsForDamage.length > 0) {
        clusterNewPositionsForDamage.forEach((itemPositons, index) => {
          var line = new Feature({
            geometry: new MultiLineString(itemPositons),
          });
          var vectorSource = new VectorSource({
          ...
          this.lineNewFeatureListForDamage.push(line);
          this.lineNewVectorForDamage = new VectorLayer({
            source: vectorSource,
            name: "lineNew",
            style: new Style({
              stroke: new Stroke({
          ...
          this.map.addLayer(this.lineNewVectorForDamage);
          this.lineNewVectorForDamage.setVisible(this.switchValue);
    ...
    */
    ...
    updateLineLayer() {
      ...
      if (this.switchValue) {
        //更新线条图层位置
        if (clusterNewPositions != null) {
          clusterNewPositions.forEach((itemPositons, index) => {
            this.lineNewFeatureList[index].getGeometry().setCoordinates(itemPositons);
          });
        }
        if (clusterNewPositionsForDamage != null) {
          clusterNewPositionsForDamage.forEach((itemPositons, index) => {
            this.lineNewFeatureListForDamage[index].getGeometry().setCoordinates(itemPositons);
          });
        }
        //显示最新线条图层
        this.lineNewVector.setVisible(this.switchValue);
    ...
    */
    ...
    handleSwitchChange(value) {
      console.log('Switch changed to:', value ? 'on' : 'off');
      if (!value) {
        this.lineNewVector.setVisible(value);
        this.lineNewVectorForDamage.setVisible(value);
      } 
    },
