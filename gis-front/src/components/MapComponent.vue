  ...
  export default {  
    ...
    methods:{
      async fetchData(){
        ...
        try{
          ...
          console.log("这是从后端拿到的数据啊：",this.data);
        }catch(error){
    ...
    this.dynamic_source = new VectorSource();
      ...
      // });  
        ...
        // ----------------------------------------------------------集群~
      const  cluster_feature = new Array(1000);

      for(let i=0;i<1000;i++){
        const coordinate_cluster = [Math.random() * 360 % 90 ,Math.random()*90 % 50 -60 ] ;
        cluster_feature[i] = new Feature(new Point(coordinate_cluster));
      }

      const source_cluster = new VectorSource({
        features: cluster_feature,
      });
