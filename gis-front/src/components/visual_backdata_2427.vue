  ...
  methods: {
    ...
    initWebSocket(){
        ...
        this.ws.onmessage = (event) => {
          ...
          console.log('Message from server:', this.message.ellipseList);
          this.updateCanvas();
        };
    ...
    // 假设我们有一个函数可以绘制椭圆到Canvas上  
    drawEllipseToCanvas(ctx, centerX, centerY, radiusX, radiusY) {  
        ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height); // 清除之前的绘制  
        ctx.save();  
        ctx.beginPath();  
        ctx.ellipse(centerX, centerY, radiusX, radiusY, 0, 0, 2 * Math.PI); // 注意：这行代码在某些浏览器中可能不被支持  
        // 如果浏览器不支持ctx.ellipse，则需要使用更复杂的方法来绘制椭圆  
        ctx.strokeStyle = 'red';  
        ctx.stroke(); // 描边椭圆，如果你想填充则使用ctx.fill()并设置fillStyle  
        ctx.restore();  
     },

      // 创建一个函数，该函数在请求时绘制椭圆并返回Canvas元素  
    getCanvasWithEllipse(extent, resolution, pixelRatio, size, projection) {  
      this.canvasS = document.createElement('canvas');  
      const ctx = this.canvasS.getContext('2d');  
      this.canvasS.width = size[0];  
      this.canvasS.height = size[1];  
        
      // 假设我们要在Canvas的中心绘制一个椭圆  
      const centerX = canvas.width / 2;  
      const centerY = canvas.height / 2;  
      const radiusX = canvas.width / 4; // 举例：椭圆长轴为Canvas宽度的一半  
      const radiusY = canvas.height / 3; // 举例：椭圆短轴为Canvas高度的三分之一  
    ...
    drawEllipse4(cluster) {
      ...
      pointfeatures.forEach(function (feature) {
        if (feature.getGeometry().getType() === 'Point') {
          ...
          if (nodeslist.includes(featureProperties.vehicleId)) {
            // console.log("featureProperties.vehicleId:" + featureProperties.vehicleId);
            minX = Math.min(minX, featureProperties.longitude); // 经度  
            maxX = Math.max(maxX, featureProperties.longitude);
            minY = Math.min(minY, featureProperties.latitude); // 纬度  
            maxY = Math.max(maxY, featureProperties.latitude);
          }
        }
      });
